...
def render( scene ) :
    ...
    tdeb = time.time()
    # Calcul des couleurs attribuées à chaque pixel de l'image :
    for iy in range(start,end):
        print(f"Calcule de la ligne {iy}",rank)
        
        yy = (1 - ((iy + 0.5) * invHeight)) * angle
        for ix in range(width):
            ...
            pixels[iy, ix, :] = trace(ray, 0, scene )
    ...
    print(pixels[start:end,:].shape, rank)
    recvbuf = comm.gather(pixels[start:end,:], root=0)

    if rank ==0:
        print("recvbuf",len(recvbuf))
        recvbuf_list=np.vstack(recvbuf)
        print("--------recvbuf_list-----------",recvbuf_list.shape)

    tfin = time.time()
    print(f"Temps pris pour calculer l'image : {tfin-tdeb} secondes", rank)
    # Sauvegarde de l'image calculée 
    if rank ==0:
        formatted = (recvbuf_list * 255).astype('uint8')
        image = Image.fromarray(formatted)
        image.save("scenenaiv.png")
