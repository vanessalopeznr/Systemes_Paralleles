...
std::vector<int>
computeMandelbrotSet( int W, int H, int HLoc, int maxIter, int firstRow )
{
    ...
    for ( int i = 0; i < HLoc; ++i ) {
      computeMandelbrotSetRow(W, H, maxIter, i+firstRow, pixels.data() + W*i );
    }
...
int main(int argc, char *argv[] ) 
 { 
    ...
    // Calcul du nombre de lignes par noeuds de calcul :
    int HLoc = H/nbp;
    std::vector<int> HLocs(nbp, HLoc);
    for (int iProc=0; iProc < H%nbp; ++iProc)
        HLocs[iProc] += 1;
    std::vector<int> firstRows(nbp);
    firstRows[0] = 0;
    for (int iProc=1; iProc < nbp; ++iProc)
        firstRows[iProc] = firstRows[iProc-1] + HLocs[iProc-1];
    // Calcul de la première ligne prise en compte par le noeud de calcul :
    int firstRow = firstRows[rank];

    out << "Calcul entre les lignes " << firstRow << " et " <<  firstRow + HLocs[rank] << std::endl;
    auto iters = computeMandelbrotSet( W, H, HLocs[rank], maxIter, firstRow );
    auto globIters = std::vector<int>(W*H);
    HLocs[0] *= W;
    for (int iProc=1; iProc < nbp; ++iProc)
    {
        firstRows[iProc] *= W;
    ...
    MPI_Gatherv(iters.data(), HLocs[rank], MPI_INT, globIters.data(), HLocs.data(), firstRows.data(), MPI_INT, 0, global );
    if (rank==0)
        savePicture("mandelbrot.tga", W, H, globIters, maxIter);
