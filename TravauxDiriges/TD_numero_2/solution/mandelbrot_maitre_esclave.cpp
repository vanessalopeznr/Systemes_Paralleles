...
std::vector<int>
computeMandelbrotSet( int W, int H, int maxIter, MPI_Comm com )
{
    std::chrono::time_point<std::chrono::system_clock> start, end;
    std::vector<int> pixels;
    int rank, nbp;
    MPI_Comm_rank(com, &rank);
    MPI_Comm_size(com, &nbp );
    start = std::chrono::system_clock::now();
    if (rank==0) // C'est le maître !
    {
        std::vector<int>(W*H).swap(pixels);
        int task = 0;
        for (int iProc=1; iProc<nbp; ++iProc)
        {
            MPI_Ssend(&task, 1, MPI_INT, iProc, 101, com);
            ++task;
        }
        MPI_Status status;
        MPI_Request request;
        ...
        while (task < H)
        {
            MPI_Recv(row.data(), W, MPI_INT, MPI_ANY_SOURCE, MPI_ANY_TAG, com, &status);
            MPI_Isend( &task, 1, MPI_INT, status.MPI_SOURCE, 101, com, &request);
            std::copy( row.begin(), row.end(), pixels.data() + W*status.MPI_TAG);
        ...
        int task;
        do {
            MPI_Recv(&task, 1, MPI_INT, 0, MPI_ANY_TAG, com, &status);
            if (task >= 0) {
                computeMandelbrotSetRow( W, H, maxIter, task, row.data());
                MPI_Ssend(row.data(), W, MPI_INT, 0, task, com);
            }
        } while(task!=-1);
    }
...
int main(int argc, char *argv[] ) 
 { 
    ...
    auto iters = computeMandelbrotSet( W, H, maxIter, global );
    if (rank==0)
