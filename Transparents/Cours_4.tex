...
\begin{frame}[fragile]{Generalization for more processes}
  ...
  \begin{block}{\small Nested bissection}
    \begin{itemize}
      ...
      \item The factorization can be compute recursively
  ...
  \begin{exampleblock}{\small Properties}
    \begin{itemize}
      \item For square or rectangular domains (image per example), \\ideal algorithm to sparsify the factorization;
      \item Drawback : $A$ can be inversible but a matrix $A_{ii}$ could be\\ not inversible
    \end{itemize}
  \end{exampleblock}
  \end{multicols}
