...
\usepackage{hyperref}
\usetikzlibrary{shadows, arrows, decorations.pathmorphing, fadings, shapes.arrows, positioning, calc, shapes, fit, matrix,math, tikzmark}
...
\begin{itemize}
    ...
    \item How to change this algorithm for distributed parallel architecture ?
\end{itemize}
  \end{alertblock}
\end{multicols}
\end{frame}
...
\begin{block}{Algorithm}
    ...
    \begin{itemize}
        \item Setup an array of initially empty "buckets"
        \item \textbf{Scatter} : Put each element of the list in the right bucket
        \item Sort the elements inside each bucket 
        \item \textbf{Gather} : Visit each bucket in order and put all elements back in original list
    \end{itemize}

    \begin{figure}
    \begin{tikzpicture}
        \foreach \x / \p in { 0.13/0,0.25/1,0.15/2,0.43/3,0.23/4,0.49/5,0.9/6,0.85/7,0.75/8, 0.71/9}
        {
            \node[draw, inner sep=1pt] (E\p) at (0.5*\p,0.) {\tiny \x};
            %\node[draw, circle, inner sep=1pt] (P\r) at (0.6*\y,-0.6*\x) {\tiny $P_{\r}$};
        }
        \node at (5.5,0.) {\tiny Values};
        \foreach \x / \v in { 0/0., 1/0.1, 2/0.2, 3/0.3, 4/0.4, 5/0.5, 6/0.6, 7/0.7, 8/0.8, 9/0.9, 10/1.} {
            \draw (0.5*\x, -0.75) -- (0.5*\x,-0.5);
            \node at (0.5*\x, -0.35) {\tiny \textcolor{blue}{\v}};
        }
        \node at (5.5,-0.62) {\tiny Buckets};
        \node at (0.75,-0.5) {\textcolor{orange}{\tiny 0.13}};
        \node at (0.75,-0.7) {\textcolor{orange}{\tiny 0.15}};

        \node at (1.25,-0.5) {\textcolor{orange}{\tiny 0.25}};
        \node at (1.25,-0.7) {\textcolor{orange}{\tiny 0.23}};

        \node at (2.25,-0.5) {\textcolor{orange}{\tiny 0.43}};
        \node at (2.25,-0.7) {\textcolor{orange}{\tiny 0.49}};

        \node at (3.75,-0.5) {\textcolor{orange}{\tiny 0.75}};
        \node at (3.75,-0.7) {\textcolor{orange}{\tiny 0.71}};

        \node at (4.25,-0.5) {\textcolor{orange}{\tiny 0.85}};

        \node at (4.75,-0.5) {\textcolor{orange}{\tiny 0.9}};

        \foreach \x / \p in { 0.13/0,0.15/1,0.23/2,0.25/3,0.43/4,0.49/5,0.71/6,0.75/7,0.85/8, 0.9/9}
        {
            \node[draw, inner sep=1pt] (E\p) at (0.5*\p,-1.) {\tiny \textcolor{red}{\x}};
            %\node[draw, circle, inner sep=1pt] (P\r) at (0.6*\y,-0.6*\x) {\tiny $P_{\r}$};
        }
        \node at (5.5,-1.) {\tiny Sorted values};
    \end{tikzpicture}
\end{figure}
\begin{itemize}
